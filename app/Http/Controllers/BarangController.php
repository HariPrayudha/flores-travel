...
class BarangController extends Controller
{
    ...
    public function exportSupir(Request $request)
    {
        $validated = $request->validate([
            'ids'   => ['required', 'array', 'min:1'],
            'ids.*' => ['integer', 'exists:barangs,id'],
        ...
        try {
            return DB::transaction(function () use ($validated, $user) {
                $barangs = Barang::whereIn('id', $validated['ids'])->lockForUpdate()->get();
                ...
                $items = [];
                $total = $lunas = $belum = $transfer = 0;

                foreach ($barangs as $b) {
                    $nom = (float) ($b->harga_terbayar ?? $b->harga_awal ?? 0);
                    $total += $nom;
                    $st = strtolower($b->status_bayar ?? '');
                    if ($st === 'lunas') $lunas += $nom;
                    elseif ($st === 'belum bayar') $belum += $nom;
                    elseif ($st === 'transfer') $transfer += $nom;
                    ...
                    $items[] = [
                        ...
                        'tujuan' => $b->paket_antar ? ($b->alamat ?? '') : $b->kota_tujuan,
                ...
                $now = now();
                foreach ($barangs as $b) {
                    $b->exported_supir = true;
                    $b->exported_at_supir = $now;
                    $b->surat_jalan_batch_id = $batch->id;
                    $b->user_update = $user->id;
                    $b->save();
                }

                return response()->json([
                    'success' => true,
                    'message' => 'Batch surat jalan disimpan.',
                    'data'    => [
                        'batch' => $batch,
                        'exported_ids' => $barangs->pluck('id')->values(),
                    ],
                ], 201);
            });
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal membuat batch surat jalan.',
                'error'   => $e->getMessage(),
            ], 500);
        }
    }
